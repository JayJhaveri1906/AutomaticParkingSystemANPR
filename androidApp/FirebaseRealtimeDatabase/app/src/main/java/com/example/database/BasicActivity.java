...
public class BasicActivity extends AppCompatActivity {
	...
	private DatabaseReference mRootRef, mUsersRef, mMessageRef , mCostRef , mParkingSlotRef , mParkingTimeRef ;
	...
	protected void onCreate(Bundle savedInstanceState) {
        ...
        mCostRef = mRootRef.child(CHILD_MESSAGES);
        mParkingSlotRef = mRootRef.child(CHILD_MESSAGES);
        mParkingTimeRef = mRootRef.child(CHILD_MESSAGES);
...
//					FriendlyMessage friendlyMessage = dataSnapshot.child(CHILD_MESSAGES).child(key).getValue(FriendlyMessage.class);
					String cost=dataSnapshot.child(CHILD_MESSAGES).child(key).child("Cost").getValue().toString();
  				    String uname=dataSnapshot.child(CHILD_MESSAGES).child(key).child("username").getValue().toString();
					String noplate=dataSnapshot.child(CHILD_MESSAGES).child(key).child("text").getValue().toString();
                    String parkingTime=dataSnapshot.child(CHILD_MESSAGES).child(key).child("Parking time").getValue().toString();
                    String partingSlot=dataSnapshot.child(CHILD_MESSAGES).child(key).child("Parking Slot").getValue().toString();

					mTextView.append("Username: " + uname + " \n");
					mTextView.append("NumberPlate: " + noplate + " \n");
                    mTextView.append("Parking Time: " + parkingTime + "\n");
                    mTextView.append("Parking Slot: ₹" + partingSlot + "\n");
					mTextView.append("Cost: ₹" + cost + "\n");

//					long now = System.currentTimeMillis();
//					xlong past = now - (60 * 60 * 24 * 45 * 1000L);
		...
		mButtonPush.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View view) {
				...
				if (TextUtils.isEmpty(message)) {
					...
					mMessageRef.child(message).setValue(friendlyMessage);
                    mParkingSlotRef.child(message).child("Parking Slot").setValue("A1");
                    mParkingTimeRef.child(message).child("Parking time").setValue("0");
                    mCostRef.child(message).child("Cost").setValue("0");
